...
final class AsaasHttpClient implements AsaasClientInterface,AsaasPaymentClientInterface
{
 ...
 private function request(string $method,string $path,array $payload=[],array $query=[]):array
 {
  ...
  $url=$this->baseUrl.$path.($query?'?'.http_build_query($query):'');$ch=curl_init($url);$headers=['accept: application/json','content-type: application/json','access_token: '.$this->apiKey,'User-Agent: AlugFacil/1.0 ('.$this->environment.')'];
