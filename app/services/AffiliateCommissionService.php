...
final class AffiliateCommissionService
{
    ...
    private const REVERSAL_EVENTS=['PAYMENT_DELETED','PAYMENT_REFUNDED','PAYMENT_PARTIALLY_REFUNDED','PAYMENT_REFUND_IN_PROGRESS','PAYMENT_CHARGEBACK_REQUESTED','PAYMENT_CHARGEBACK_DISPUTE','PAYMENT_AWAITING_CHARGEBACK_REVERSAL'];
    ...
    {
        $config=$this->db->query('SELECT percentual_bps FROM configuracoes_comissao_afiliados WHERE id=1 FOR SHARE')->fetch(PDO::FETCH_ASSOC);
        if(!$config)throw new RuntimeException('Configuracao global de comissao nao encontrada.');
        $base=(int)$origin['valor_centavos'];$bps=(int)$config['percentual_bps'];$commission=self::calculateCommission($base,$bps);
    ...
    public function summary(int $affiliateId,?DateTimeImmutable $asOf=null): array
    {
        ...
        $s=$this->db->prepare("SELECT c.*,COALESCE((SELECT SUM(a.valor_centavos) FROM alocacoes_pagamentos_afiliados a WHERE a.comissao_afiliado_id=c.id),0) AS alocado FROM comissoes_afiliados c WHERE c.afiliado_id=:afiliado");$s->execute(['afiliado'=>$affiliateId]);
        $open=0;$grossAvailable=0;
        ...
        return ['em_aberto_centavos'=>$open,'disponivel_bruto_centavos'=>$grossAvailable,'ajustes_centavos'=>$adjustments,'saldo_disponivel_centavos'=>$grossAvailable+$adjustments,'total_pago_centavos'=>$paid];
    }
    ...
    {
        $sql="SELECT c.*,COALESCE((SELECT SUM(a.valor_centavos) FROM alocacoes_pagamentos_afiliados a WHERE a.comissao_afiliado_id=c.id),0) AS alocado_centavos FROM comissoes_afiliados c WHERE c.afiliado_id=:afiliado AND c.status IN ('EM_ABERTO','DISPONIVEL') AND c.estornada_em IS NULL AND c.disponivel_em<=:agora ORDER BY c.disponivel_em,c.id FOR UPDATE OF c";
