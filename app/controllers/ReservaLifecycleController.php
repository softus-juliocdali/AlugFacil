...
final class ReservaLifecycleController extends Controller
{
    ...
    {
        Auth::requireGuest();verify_csrf();$rid=$this->id($id);$m=new Reserva();$r=$m->buscarDetalheCliente($rid,(int)Auth::user()['id']);if(!$r)$this->naoEncontrada();
        $motivo=$this->motivo(false);
        try{$resultado=(new CancelamentoReservaService())->cancelarUsuario($rid,(int)Auth::user()['id'],null,$motivo);flash('success',empty($resultado['reembolso_id'])?'Reserva cancelada.':'Reserva cancelada e reembolso solicitado.');}catch(Throwable$e){flash('error',\App\Services\FinancialErrorMessage::publicMessage($e,'Nao foi possivel concluir a operacao.'));}
    ...
    public function adminTransicao(string $id):void { Auth::requireRole('admin');verify_csrf();$rid=$this->id($id);$r=(new Reserva())->buscarDetalheAdministrativo($rid);if(!$r)$this->naoEncontrada();$acao=(string)($_POST['acao']??'');if($r['schema_financeiro']===2&&in_array($acao,['cancelar','aprovar_cancelamento'],true)){try{(new \App\Services\ExceptionalReservationCancellationService())->cancel($rid,(int)Auth::user()['id'],(string)($_POST['motivo']??''),(string)($_POST['politica_financeira']??''));flash('success','Cancelamento excepcional registrado. Valores dependem da politica e conciliacao administrativa; nenhum estorno automatico foi emitido.');}catch(Throwable $e){flash('error',\App\Services\FinancialErrorMessage::publicMessage($e,'Nao foi possivel registrar a excecao.'));}$this->redirect('/admin/reservas/'.$rid);}$novo=match($acao){'cancelar'=>'cancelada','aprovar_cancelamento'=>'cancelada','recusar_cancelamento'=>$this->statusRetorno($r),'iniciar'=>'em_andamento','finalizar'=>'finalizada',default=>''};if($novo===''){flash('error','Acao invalida.');$this->redirect('/admin/reservas/'.$rid);}$this->mudar($rid,$novo,'administrador',(int)Auth::user()['id'],$this->motivo(true),'/admin/reservas/'.$rid); }

    private function mudar(int $id,string $novo,string $tipo,int $uid,?string $motivo,string $volta):never {try{(new ReservaStatusService())->transicionar($id,$novo,['motivo'=>$motivo,'origem'=>$tipo==='administrador'?'administrador':'proprietario','responsavel_tipo'=>$tipo,'responsavel_id'=>$uid]);flash('success','Status da reserva atualizado.');}catch(Throwable $e){app_log('Falha privada na transicao da reserva.');flash('error','Transicao nao permitida para o status atual.');}$this->redirect($volta);}
