...
function old(string $key, string $default = '', string $form = 'default'): string
{
    return (string) ($_SESSION['_old'][$form][$key] ?? $default);
}

function set_old(array $data, string $form = 'default'): void
{
    if (!isset($_SESSION['_old']) || !is_array($_SESSION['_old'])) {
        $_SESSION['_old'] = [];
    }

    $_SESSION['_old'][$form] = $data;
}
