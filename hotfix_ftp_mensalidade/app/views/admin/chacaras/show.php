...
<?php if(isset(($transicoes[$chacara['status_aprovacao']]??[])['aprovada'])): ?><fieldset class="form-full"><legend>Mensalidade na aprovacao</legend><label><input type="radio" name="mensalidade" value="sem" required> Sem mensalidade</label><label><input type="radio" name="mensalidade" value="com" required> Com mensalidade</label><small>Sem mensalidade publica o anuncio imediatamente. Com mensalidade, a publicacao aguarda a confirmacao do pagamento.</small></fieldset><label class="form-full" data-approval-monthly-value>Valor mensal do anuncio (R$) <input type="text" inputmode="decimal" name="valor_mensal" value="<?= e(number_format($valorSugerido/100,2,',','')) ?>"><small>Obrigatorio somente ao aprovar com mensalidade.</small></label><?php endif; ?>
...
<div><dt>Status</dt><dd><?= e($label($mensalidade['status']??'SEM_MENSALIDADE')) ?></dd></div><div><dt>Valor</dt><dd><?= e($money(isset($mensalidade['valor_centavos'])?(int)$mensalidade['valor_centavos']:null)) ?></dd></div><div><dt>Proximo vencimento</dt><dd><?= e($date($mensalidade['proximo_vencimento']??null)) ?></dd></div><div><dt>Ultimo pagamento</dt><dd><?= e($date($mensalidade['ultimo_pagamento_em']??null,true)) ?></dd></div><div><dt>Assinatura Asaas</dt><dd><?= e($mensalidade['asaas_subscription_id']??'-') ?></dd></div></dl>
<?php if(!empty($mensalidade['ultima_falha_sincronizacao'])): ?><div class="alert alert-error"><?= e($mensalidade['ultima_falha_sincronizacao']) ?> Salve novamente para tentar conciliar sem criar outra assinatura.</div><?php endif; ?>
<form class="owner-property-form" method="post" action="<?= url('/admin/chacaras/'.(int)$chacara['id'].'/mensalidade') ?>" onsubmit="return confirm('Confirma a alteracao da mensalidade deste anuncio?')"><?= csrf_field() ?><label><input type="radio" name="mensalidade" value="sem" <?= empty($mensalidade['ativa_flag'])?'checked':'' ?>> Sem mensalidade</label><label><input type="radio" name="mensalidade" value="com" <?= !empty($mensalidade['ativa_flag'])?'checked':'' ?>> Com mensalidade</label><label class="form-full">Valor mensal (R$) <input type="text" inputmode="decimal" name="valor_mensal" value="<?= e(number_format($valorSugerido/100,2,',','')) ?>"></label><button class="btn btn-primary" type="submit">Salvar mensalidade</button></form></article>
...
(() => {
    ...
    const syncRequired = () => {
        ...
        valueInput.required = selected === 'com';
        valueInput.disabled = selected === 'sem';
    };
