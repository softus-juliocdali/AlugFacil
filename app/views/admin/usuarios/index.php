...
<div class="panel-page-heading">
    <div>
        ...
        <h1>Usu&aacute;rios clientes</h1>
        <p>Consulte clientes, hist&oacute;rico de reservas e controle de acesso.</p>
    </div>
</div>

<section class="panel-card admin-filter-card">
    <form class="admin-search-form" method="get" action="<?= url('/admin/usuarios') ?>">
        <label>
        ...
        <?php if ($busca !== ''): ?>
            <a class="btn btn-outline" href="<?= url('/admin/usuarios') ?>">Limpar</a>
        <?php endif; ?>
    </form>
</section>
...
<section class="panel-card">
    ...
    <?php if (empty($usuarios)): ?>
        <div class="empty-preview">
            <span>&#9633;</span>
            <div>
                <strong>Nenhum usu&aacute;rio encontrado</strong>
            ...
            <table class="panel-table">
                ...
                <tbody>
                    <?php foreach ($usuarios as $usuario): ?>
                        <?php $novoStatus = $usuario['status'] === 'bloqueado' ? 'ativo' : 'bloqueado'; ?>
                        <tr>
                            <td><?= e($usuario['nome']) ?></td>
                            <td><?= e($usuario['telefone'] ?: '-') ?></td>
                            <td><?= e($usuario['email']) ?></td>
                            <td><?= e((string) (int) $usuario['total_reservas']) ?></td>
                            <td><span class="status-pill"><?= e($formatarStatus($usuario['status'])) ?></span></td>
                            <td><?= e($formatarData($usuario['data_cadastro'])) ?></td>
                            <td>
                                <div class="admin-table-actions">
                                    <a class="btn btn-outline btn-small" href="<?= url('/admin/usuarios/' . (int) $usuario['id']) ?>">Detalhes</a>
                                    <form method="post" action="<?= url('/admin/usuarios/' . (int) $usuario['id'] . '/status') ?>">
                                        <?= csrf_field() ?>
