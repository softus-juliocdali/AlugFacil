...
final class AffiliatePortalService
{
    ...
    public function commissions(int$affiliateId,array$filters=[],int$page=1,int$perPage=30,?DateTimeImmutable$now=null):array
    {
        ...
        $status=strtoupper(trim((string)($filters['status']??'')));if(in_array($status,['BLOQUEADA','EM_ABERTO','DISPONIVEL','PAGA','ESTORNADA','CANCELADA'],true)){$where[]='x.status_exibicao=:status';$params['status']=$status;}
        ...
        $cte="WITH dados AS(SELECT c.id,c.afiliado_id,c.proprietario_id,c.chacara_id,c.confirmado_em,c.disponivel_em,c.percentual_bps,c.valor_comissao_centavos,p.nome AS proprietario_nome,ch.nome AS chacara_nome,CASE WHEN c.estornada_em IS NOT NULL AND c.status<>'PAGA' THEN 'ESTORNADA' WHEN c.status IN ('PAGA','ESTORNADA','CANCELADA') THEN c.status WHEN EXISTS(SELECT 1 FROM cobrancas_mensalidades cm WHERE cm.id=c.cobranca_mensalidade_id AND cm.refund_bloqueado) THEN 'BLOQUEADA' WHEN c.disponivel_em<=CAST(:agora AS TIMESTAMPTZ) THEN 'DISPONIVEL' ELSE 'EM_ABERTO' END AS status_exibicao FROM comissoes_afiliados c INNER JOIN proprietarios p ON p.id=c.proprietario_id INNER JOIN chacaras ch ON ch.id=c.chacara_id) SELECT * FROM dados x WHERE ".implode(' AND ',$where);
